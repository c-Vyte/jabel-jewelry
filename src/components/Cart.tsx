...
export default function Cart({ isOpen, onClose, cart, setCart }: CartProps) {
  ...
  const sendWhatsAppNotification = (order: Order, customMessage?: string) => {
    let message = customMessage || `*Order ${order.status.toUpperCase()} - #${order.id}*\n\n`;
  ...
  const sendEmailNotification = (order: Order) => {
    ...
    const subject = encodeURIComponent(`Order ${order.status.toUpperCase()} - #${order.id}`);
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            />
            <motion.div 
              variants={cartVariants.checkout}
              initial="initial"
              animate="animate"
              exit="exit"
              transition={cartTransition}
              ...
              {cart.length === 0 ? (
                ...
                >
                  {cart.map((item, index) => (
                    <motion.div
                      key={item.id}
                      variants={cartVariants.item}
                      initial="initial"
                      animate="animate"
                      exit="exit"
                      transition={{ ...cartTransition, delay: index * 0.06 }}
            ...
            {cart.length > 0 && (
              <div className="p-6 border-t border-theme-border bg-theme-bg/50 space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-medium text-theme-text">GH₵{cartTotal.toLocaleString()}</span>
                </div>
                ...
                <button 
                  onClick={() => {
                    const order = createOrder();
                    saveOrder(order);
                    sendWhatsAppNotification(order, `*Quick Order - #${order.id}*\n\n`);
                    setCart([]);
                    onClose();
